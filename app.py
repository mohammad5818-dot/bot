...
async def share_to_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # پرامپت کاربر را در کپشن می‌گذاریم، اما برای جلوگیری از طولانی شدن بیش از حد، آن را محدود می‌کنیم
    caption = f"توضیحات کاربر: {user_prompt[:500]}..." 

    try:
        # پرانتز بسته شدن تابع send_photo اصلاح شد
...
async def handle_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⭐ اصلاحیه: محدود کردن طول پرامپت برای جلوگیری از خطای BadRequest (طول متن)
    ...
    if current_state == 1:
        ...
        if client:
            try:
                ...
                # حذف 'mime_type' برای رفع خطای unexpected keyword argument
...
def main():
    ...
    if not WEBHOOK_URL:
        # اصلاح نهایی برای رفع خطای SyntaxError: unterminated string literal
